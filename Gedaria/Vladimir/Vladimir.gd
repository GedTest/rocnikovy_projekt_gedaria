...
# ------------------------------------------------------------------------------
func Move(): # MOVE
	if Input.is_action_pressed("right") && !bRake:
		direction = 1
		attackDirection = 1
		$Sprite.flip_h = false
		
	elif Input.is_action_pressed("left") && !bRake:
		direction = -1
		attackDirection = -1
		$Sprite.flip_h = true
...
func HeavyAttack(): # HEAVY ATTACK, slow but high dmg
	if Input.is_action_just_pressed("heavy attack") && !bSwording && !bBlocking:
...
# ------------------------------------------------------------------------------
func Shoot():
	if Input.is_action_pressed("shoot") && pebbleCounter > 0 && !bCrouching:
		bCanMove = false
		bAimining = true
...
func Rake(): # RAKE LEAVES TO CREATE PILE OF LEAVES
	...
	elif Input.is_action_just_pressed("attack") && !bCrouching:
		StateMachine.travel('ATTACK')
# ------------------------------------------------------------------------------
func Hit(var dmg):
	if !bIsDead:
...
func _draw(): # ENGINE DRAW FUNCTION
	if bAimining:
		draw_line(Vector2(0,0),mousePosition,Color(1,1,1,0.3),8.0)
