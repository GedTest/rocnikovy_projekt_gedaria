...
const SAVE_PATH = 'user://ss3.json'

func _ready():
	currentScene = Save_Load.GetScene()
	$Label1.text = Save_Load.SaveSlot3[0]
	$Label2.text = Save_Load.SaveSlot3[1]
	$Label3.text = Save_Load.SaveSlot3[2]
	bEmpty = Save_Load.SaveSlot3[3]

func _on_SS3_pressed():
	if currentScene != "res://Level/MainMenu/MainMenu.tscn":
		...
		Save_Load.SaveSlot3[0] = $Label1.text
		Save_Load.SaveSlot3[1] = $Label2.text
		Save_Load.SaveSlot3[2] = $Label3.text
		Save_Load.SaveSlot3[3] = bEmpty
		Save_Load.SaveSlot3[4] = Save_Load.get_level()
