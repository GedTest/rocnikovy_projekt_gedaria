...
const SAVE_PATH = 'user://ss2.json'

func _ready():
	currentScene = Save_Load.GetScene()
	$Label1.text = Save_Load.SaveSlot2[0]
	$Label2.text = Save_Load.SaveSlot2[1]
	$Label3.text = Save_Load.SaveSlot2[2]
	bEmpty = Save_Load.SaveSlot2[3]

func _on_SS2_pressed():
	if currentScene != "res://Level/MainMenu/MainMenu.tscn":
		...
		Save_Load.SaveSlot2[0] = $Label1.text
		Save_Load.SaveSlot2[1] = $Label2.text
		Save_Load.SaveSlot2[2] = $Label3.text
		Save_Load.SaveSlot2[3] = bEmpty
		Save_Load.SaveSlot2[4] = Save_Load.get_level()
